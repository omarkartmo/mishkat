...
| **28** | `getSearchHistory(studentId)` | `App.tsx` (Search history) | Client Component State | None (Transient UI session) | Managed as ephemeral React state; StorageService removed. | **DECOMMISSIONED (Client-only state)** |
| **29** | `addSearchHistory(studentId, query)` | `SearchResultsView.tsx` | Client Component State | None (Transient UI session) | Managed as ephemeral React state; StorageService removed. | **DECOMMISSIONED (Client-only state)** |
| **30** | `removeSearchHistoryItem` / `clearSearchHistory` | `SearchResultsView.tsx` | Client Component State | None (Transient UI session) | Managed as ephemeral React state; StorageService removed. | **DECOMMISSIONED (Client-only state)** |
| **31** | `getOpenBooks(studentId)` / `addOpenBook` / `closeOpenBook` | `App.tsx`, `StudentPortalView` | Client Component State | None (Transient UI tabs) | Managed as ephemeral React state in `activeReadingBook`; StorageService removed. | **DECOMMISSIONED (Client-only state)** |
| **32** | `getStudents()` / `getUsers()` | `App.tsx` (initial state, `StudentManagerView`, `LoanManagerView`) | `StorageService.students` | `GET /api/v1/users` | Migrated to `UserRepository.getUsers()`. | **MIGRATED (Phase 6.2 - Users)** |
| **33** | `addStudent(student)` / `addUser(user)` | `App.tsx` (`StudentManagerView`) | `StorageService.students` | `POST /api/v1/users` | Migrated to `UserRepository.createUser()`. | **MIGRATED (Phase 6.2 - Users)** |
| **34** | `importStudentsCSV(csv)` / `bulkImportStudents(roster)` | `App.tsx` (`StudentManagerView`) | `StorageService.students` | `POST /api/v1/users/roster-import` | Migrated to `UserRepository.bulkImportStudents()`. | **MIGRATED (Phase 6.2 - Users)** |
| **35** | `updateStudent(id, updates)` | `App.tsx` (`StudentManagerView`) | `StorageService.students` | `PUT /api/v1/users/:id` | Migrated to `UserRepository.updateUser()`. | **MIGRATED (Phase 6.2 - Users)** |
| **36** | `resetUserPassword(studentId, newPassword)` | `App.tsx` (`StudentManagerView`) | `StorageService.students` | `POST /api/v1/users/:id/reset-password` | Migrated to `UserRepository.resetPassword()`. | **MIGRATED (Phase 6.2 - Users)** |
| **37** | `getPortals()` | `App.tsx` (initial state, `WhitelistedPortalsView`) | `StorageService.portals` | `GET /api/v1/portals` | Migrated to `PortalRepository.getPortals()`. | **MIGRATED (Phase 6.1 - Portals)** |
| **38** | `addPortal(portal)` / `addWhitelistedPortal(portal)` | `App.tsx` (`WhitelistedPortalsView`) | `StorageService.portals` | `POST /api/v1/portals` | Migrated to `PortalRepository.createPortal()`. | **MIGRATED (Phase 6.1 - Portals)** |
| **39** | `updatePortal(id, updates)` / `updateWhitelistedPortal(id, updates)` | `App.tsx` (`WhitelistedPortalsView`) | `StorageService.portals` | `PUT /api/v1/portals/:id` | Migrated to `PortalRepository.updatePortal()`. | **MIGRATED (Phase 6.1 - Portals)** |
| **40** | `deletePortal(id)` / `deleteWhitelistedPortal(id)` | `App.tsx` (`WhitelistedPortalsView`) | `StorageService.portals` | `DELETE /api/v1/portals/:id` | Migrated to `PortalRepository.deletePortal()`. | **MIGRATED (Phase 6.1 - Portals)** |
| **41** | `togglePortalFeatured(id)` | `App.tsx` (`WhitelistedPortalsView`) | `StorageService.portals` | `PUT /api/v1/portals/:id` | Migrated to `PortalRepository.togglePortalFeatured()`. | **MIGRATED (Phase 6.1 - Portals)** |
| **42** | `getSubmissions()` | `App.tsx` (initial state, `ReviewQueueView`) | `StorageService.submissions` | `GET /api/v1/submissions` | Migrated to `SubmissionRepository.getSubmissions()`. Automatically scoped to current user. | **MIGRATED (Phase 1.7 - Submissions)** |
| **43** | `submitBookForReview(data)` / `addSubmission(data)` | `App.tsx` (`BookIngestionModal`) | `StorageService.submissions` | `POST /api/v1/submissions` | Migrated to `SubmissionRepository.createSubmission()`. | **MIGRATED (Phase 1.7 - Submissions)** |
| **44** | `approveSubmission(submissionId, categoryId)` | `App.tsx` (`ReviewQueueView`) | `StorageService.submissions`, `digitalBooks` | `POST /api/v1/submissions/:id/review` | Migrated to `SubmissionRepository.approveSubmission()` -> `POST /submissions/:id/review`. Creates digital book and sends notification. | **MIGRATED (Phase 1.7 - Submissions)** |
| **45** | `rejectSubmission(submissionId, reason)` | `App.tsx` (`ReviewQueueView`) | `StorageService.submissions` | `POST /api/v1/submissions/:id/review` | Migrated to `SubmissionRepository.rejectSubmission()` -> `POST /submissions/:id/review`. | **MIGRATED (Phase 1.7 - Submissions)** |
| **46** | `getStudentFavorites(studentId)` / `getFavorites(studentId)` | `App.tsx` (initial state, `FavoritesView`, `DigitalLibraryView`, `PhysicalLibraryView`) | `StorageService.favorites` | `GET /api/v1/favorites` | Migrated to `FavoriteRepository.getFavorites()`. Automatically scoped to current user. | **MIGRATED (Phase 1.7 - Favorites)** |
| **47** | `toggleFavorite(bookId, studentId)` | `App.tsx` (`FavoritesView`, Library views) | `StorageService.favorites` | `POST /api/v1/favorites/toggle` | Migrated to `FavoriteRepository.toggleFavorite()`. | **MIGRATED (Phase 1.7 - Favorites)** |
...
| **52** | `getPhysicalBookmarks(studentId)` | `App.tsx` (initial state, `ReadingWorkspaceView`, `PhysicalBookmarkModal`) | `StorageService.physicalBookmarks` | `GET /api/v1/bookmarks` | Migrated to `BookmarkRepository.getBookmarks()`. Automatically scoped to current user. | **MIGRATED (Phase 1.7 - Bookmarks)** |
| **53** | `savePhysicalBookmark(data)` | `App.tsx` (`PhysicalBookmarkModal`, `BookReaderModal`) | `StorageService.physicalBookmarks` | `POST /api/v1/bookmarks` | Migrated to `BookmarkRepository.saveBookmark()`. | **MIGRATED (Phase 1.7 - Bookmarks)** |
| **54** | `deletePhysicalBookmark(bookmarkId)` | `App.tsx` (`ReadingWorkspaceView`, `BookReaderModal`) | `StorageService.physicalBookmarks` | `DELETE /api/v1/bookmarks/:id` | Migrated to `BookmarkRepository.deleteBookmark()`. | **MIGRATED (Phase 1.7 - Bookmarks)** |
| **55** | `getBookSummaries(studentId, bookId)` | `App.tsx` (initial state, `ReadingWorkspaceView`, `BookSummaryModal`) | `StorageService.summaries` | `GET /api/v1/summaries` | Migrated to `SummaryRepository.getSummaries()`. Automatically scoped to current user. | **MIGRATED (Phase 1.7 - Summaries)** |
| **56** | `saveBookSummary(summary)` | `App.tsx` (`BookSummaryModal`) | `StorageService.summaries` | `POST /api/v1/summaries` | Migrated to `SummaryRepository.saveSummary()`. | **MIGRATED (Phase 1.7 - Summaries)** |
| **57** | `deleteBookSummary(summaryId)` | `App.tsx` (`ReadingWorkspaceView`) | `StorageService.summaries` | `DELETE /api/v1/summaries/:id` | Migrated to `SummaryRepository.deleteSummary()`. | **MIGRATED (Phase 1.7 - Summaries)** |
| **58** | `saveReadingProgress(progressData)` / `saveReadingProgressByBook(bookId, page, totalPages, studentId)` | `App.tsx` (`BookReaderModal`) | `StorageService.progress` | `POST /api/v1/reading-progress` | Migrated to `ReadingProgressRepository.saveReadingProgress()`. | **MIGRATED (Phase 1.7 - Reading Progress)** |
| **59** | `getReadingProgress(bookId, studentId)` / `getReadingProgressMap(studentId)` | `App.tsx` (initial state, `OverviewDashboard`, `StudentPortalView`) | `StorageService.progress` | `GET /api/v1/reading-progress` | Migrated to `ReadingProgressRepository.getReadingProgressMap()`. Automatically scoped to current user. | **MIGRATED (Phase 1.7 - Reading Progress)** |
| **60** | `dismissReadingProgress(bookId, studentId)` | `App.tsx` (`OverviewDashboard`, `StudentPortalView`) | `StorageService.progress` | `POST /api/v1/reading-progress/dismiss` | Migrated to `ReadingProgressRepository.dismissReadingProgress()`. | **MIGRATED (Phase 1.7 - Reading Progress)** |
| **61** | `clearCompletedReading(studentId)` | `App.tsx` (`OverviewDashboard`, `StudentPortalView`) | `StorageService.progress` | `POST /api/v1/reading-progress/clear-completed` | Migrated to `ReadingProgressRepository.clearCompletedReading()`. | **MIGRATED (Phase 1.7 - Reading Progress)** |
| **62** | `getConfig()` | `App.tsx` (initial state, `SystemSettingsView`, `LoanManagerView`) | `StorageService.config` | `GET /api/v1/settings` | Migrated to `SettingsRepository.getSettings()`. | **MIGRATED (Phase 6.3 - Settings)** |
| **63** | `updateConfig(updates)` / `saveConfig(config)` | `App.tsx` (`SystemSettingsView`) | `StorageService.config` | `PUT /api/v1/settings` | Migrated to `SettingsRepository.updateSettings()`. | **MIGRATED (Phase 6.3 - Settings)** |
...
| **67** | `exportDatabaseJSON()` | `App.tsx` (`SystemSettingsView`) | Serializes all in-memory arrays | `POST /api/v1/backups/create` | Migrated to `SettingsRepository.createBackup()` calling authoritative server backup API. | **MIGRATED (Phase 6.3 - Backups)** |
| **68** | `resetToDefaults()` | `App.tsx` (`SystemSettingsView`) | Resets in-memory arrays to `INITIAL_*` constants | `POST /api/v1/system/reset-demo` | Migrated to `SettingsRepository.resetDatabase()` calling authoritative server reset endpoint. | **MIGRATED (Phase 6.3 - Reset)** |
