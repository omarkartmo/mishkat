...
| **44** | `approveSubmission(submissionId, categoryId)` | `App.tsx` (`ReviewQueueView`) | `StorageService.submissions`, `digitalBooks` | `POST /api/v1/submissions/:id/approve` | Migrated to `SubmissionRepository.approveSubmission()`. Creates digital book and sends notification. | **MIGRATED (Phase 1.7 - Submissions)** |
| **45** | `rejectSubmission(submissionId, reason)` | `App.tsx` (`ReviewQueueView`) | `StorageService.submissions` | `POST /api/v1/submissions/:id/reject` | Migrated to `SubmissionRepository.rejectSubmission()`. | **MIGRATED (Phase 1.7 - Submissions)** |
