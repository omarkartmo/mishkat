...
| :- | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | `getPhysicalBooks()` | `App.tsx` (initial state & sync) | `StorageService.physicalBooks` (In-memory) | `GET /api/v1/books?type=physical` | Replace with `apiClient.get('/books?type=physical')` via React Query / custom hook. | NOT MIGRATED |
| **2** | `addPhysicalBook(book)` | `App.tsx` (`PhysicalLibraryView`) | `StorageService.physicalBooks` | `POST /api/v1/books` | Replace with `apiClient.post('/books', book)` and update local state upon server 201 response. | NOT MIGRATED |
| **3** | `updatePhysicalBook(id, updates)` | `App.tsx` (`PhysicalLibraryView`) | `StorageService.physicalBooks` | `PUT /api/v1/books/:id` | Replace with `apiClient.put('/books/:id', updates)` and update local state upon server 200 response. | NOT MIGRATED |
| **4** | `deletePhysicalBook(id)` | `App.tsx` (`PhysicalLibraryView`) | `StorageService.physicalBooks` | `DELETE /api/v1/books/:id` | Replace with `apiClient.delete('/books/:id')` and remove from state upon server 200 response. | NOT MIGRATED |
| **5** | `getDigitalBooks()` | `App.tsx` (initial state & sync) | `StorageService.digitalBooks` (In-memory) | `GET /api/v1/books?type=digital` | Replace with `apiClient.get('/books?type=digital')` via React Query / custom hook. | NOT MIGRATED |
| **6** | `addDigitalBook(book)` | `App.tsx` (`AddDigitalBookModal`, `BulkDigitalImportModal`, `ReviewQueueView`) | `StorageService.digitalBooks` | `POST /api/v1/books` or `POST /api/v1/books/bulk` | Replace with `apiClient.post('/books', book)` / `apiClient.post('/books/bulk', { books })`. | NOT MIGRATED |
| **7** | `incrementReadCount(bookId)` / `incrementDigitalReadCount(bookId)` | `App.tsx` (Digital Book Opening, Search result opening, Overview dashboard) | `StorageService.digitalBooks` (mutates in-memory `readCount`) | `POST /api/v1/books/:id/increment-read` | Dispatch fire-and-forget `apiClient.post('/books/:id/increment-read')`. | NOT MIGRATED |
| **8** | `getCategories()` | `App.tsx` (initial state & sync) | `StorageService.categories` (In-memory) | `GET /api/v1/categories` | Replace with `apiClient.get('/categories')`. | NOT MIGRATED |
| **9** | `addCategory(cat)` | `App.tsx` (`CategoryManagerView`) | `StorageService.categories` | `POST /api/v1/categories` | Replace with `apiClient.post('/categories', cat)`. | NOT MIGRATED |
| **10** | `updateCategory(id, updates)` | `App.tsx` (`CategoryManagerView`) | `StorageService.categories` | `PUT /api/v1/categories/:id` | Replace with `apiClient.put('/categories/:id', updates)`. | NOT MIGRATED |
| **11** | `deleteCategoryWithReassign(catId, targetCatId)` | `App.tsx` (`CategoryManagerView`) | `StorageService.categories`, `physicalBooks`, `digitalBooks` | `POST /api/v1/categories/:id/reassign-delete` or `DELETE /api/v1/categories/:id` | Replace with `apiClient.post('/categories/:id/reassign-delete', { targetCategoryId })`. | NOT MIGRATED |
| **12** | `bulkReclassifyBooks(bookIds, targetCatId)` | Helper method in `storageService.ts` | `StorageService.physicalBooks`, `digitalBooks` | `PUT /api/v1/books/:id` (in loop) or batch endpoint | Call `apiClient.put('/books/:id', { categoryId })` for each book. | NOT MIGRATED |
| **13** | `getLoans()` | `App.tsx` (initial state & sync) | `StorageService.loans` (In-memory) | `GET /api/v1/loans` | Replace with `apiClient.get('/loans')`. Filter by student on client or server. | NOT MIGRATED |
| **14** | `checkStudentBorrowEligibility(studentId)` / `canStudentBorrow(studentId)` | `App.tsx` (`LoanManagerView`) | Computes from `StorageService.loans` and `StorageService.config` | `GET /api/v1/loans/eligibility/:studentId` (or computed from `GET /loans`) | Query loan count & overdue loans from server response. | NOT MIGRATED |
| **15** | `createLoan(params)` | `App.tsx` (`LoanManagerView`) | `StorageService.loans`, `physicalBooks` | `POST /api/v1/loans` | Replace with `apiClient.post('/loans', params)`. | NOT MIGRATED |
| **16** | `extendLoan(loanId, days, notes)` | `App.tsx` (`LoanManagerView`) | `StorageService.loans` | `POST /api/v1/loans/:id/extend` | Replace with `apiClient.post('/loans/:id/extend', { additionalDays, notes })`. | NOT MIGRATED |
| **17** | `returnLoan(loanId, notes)` / `returnBook(loanId, notes)` | `App.tsx` (`LoanManagerView`) | `StorageService.loans`, `physicalBooks` | `POST /api/v1/loans/:id/return` | Replace with `apiClient.post('/loans/:id/return', { notes })`. | NOT MIGRATED |
| **18** | `getLoanRequests(studentId)` / `getPhysicalLoanRequests(studentId)` | `App.tsx` (initial state, `StudentPortalView`, `LoanManagerView`) | `StorageService.loanRequests` (In-memory) | `GET /api/v1/loan-requests` | Replace with `apiClient.get('/loan-requests')`. | NOT MIGRATED |
| **19** | `requestPhysicalLoan(params)` | `App.tsx` (`StudentLoanRequestModal`, `PhysicalLibraryView`) | `StorageService.loanRequests` | `POST /api/v1/loan-requests` | Replace with `apiClient.post('/loan-requests', params)`. | NOT MIGRATED |
| **20** | `approveLoanRequest(params)` | `App.tsx` (`LoanManagerView`) | `StorageService.loanRequests` | `POST /api/v1/loan-requests/:id/approve` | Replace with `apiClient.post('/loan-requests/:id/approve', { durationDays })`. | NOT MIGRATED |
| **21** | `rejectLoanRequest(requestId, reason)` | `App.tsx` (`LoanManagerView`) | `StorageService.loanRequests` | `POST /api/v1/loan-requests/:id/reject` | Replace with `apiClient.post('/loan-requests/:id/reject', { reason })`. | NOT MIGRATED |
| **22** | `confirmHandoverLoanRequest(requestId)` | `App.tsx` (`LoanManagerView`) | `StorageService.loanRequests`, `loans`, `physicalBooks` | `POST /api/v1/loan-requests/:id/handover` | Replace with `apiClient.post('/loan-requests/:id/handover')`. | NOT MIGRATED |
| **23** | `getNotifications(recipientId, role)` | `App.tsx` (initial state, `HeaderBar`) | `StorageService.notifications` (In-memory) | `GET /api/v1/notifications` | Replace with `apiClient.get('/notifications')`. | NOT MIGRATED |
| **24** | `addNotification(notif)` | Internal to `StorageService` loan actions | `StorageService.notifications` | Created server-side during loan events | Server automatically inserts notification during workflow actions. | NOT MIGRATED |
| **25** | `markNotificationAsRead(id)` | `App.tsx` (`HeaderBar`) | `StorageService.notifications` | `POST /api/v1/notifications/:id/read` | Replace with `apiClient.post('/notifications/:id/read')`. | NOT MIGRATED |
| **26** | `markAllNotificationsAsRead(recipientId, role)` | `App.tsx` (`HeaderBar`) | `StorageService.notifications` | `POST /api/v1/notifications/mark-all-read` | Replace with `apiClient.post('/notifications/mark-all-read')`. | NOT MIGRATED |
| **27** | `clearNotifications(recipientId, role)` | `App.tsx` (`HeaderBar`) | `StorageService.notifications` | `DELETE /api/v1/notifications/clear` | Replace with `apiClient.delete('/notifications/clear')`. | NOT MIGRATED |
...
| **48** | `getStudentNotes(studentId, bookId)` | `App.tsx` (initial state, `ReadingWorkspaceView`, `BookReaderModal`) | `StorageService.notes` | `GET /api/v1/notes` | Replace with `apiClient.get('/notes')`. | NOT MIGRATED |
| **49** | `addStudentNote(note)` | `App.tsx` (`NoteEditorModal`, `BookReaderModal`) | `StorageService.notes` | `POST /api/v1/notes` | Replace with `apiClient.post('/notes', note)`. | NOT MIGRATED |
| **50** | `updateStudentNote(noteId, updates)` | `App.tsx` (`NoteEditorModal`) | `StorageService.notes` | `POST /api/v1/notes` (or update endpoint) | Replace with `apiClient.post('/notes', updatedNote)`. | NOT MIGRATED |
| **51** | `deleteStudentNote(noteId)` | `App.tsx` (`ReadingWorkspaceView`, `BookReaderModal`) | `StorageService.notes` | `DELETE /api/v1/notes/:id` | Replace with `apiClient.delete('/notes/:id')`. | NOT MIGRATED |
...
| **63** | `updateConfig(updates)` / `saveConfig(config)` | `App.tsx` (`SystemSettingsView`) | `StorageService.config` | `PUT /api/v1/settings` | Replace with `apiClient.put('/settings', config)`. | NOT MIGRATED |
| **64** | `isAuthenticated()` / `getCurrentUser()` / `setCurrentUser(user)` | `App.tsx`, `Sidebar.tsx`, `HeaderBar.tsx` | `StorageService.currentUser`, `authenticated` | `GET /api/v1/auth/me` | Use React `AuthContext` backed by `apiClient.get('/auth/me')`. | NOT MIGRATED |
| **65** | `loginUser(regNumber, password)` | `App.tsx` (`LoginView`) | In-memory lookup / `POST /api/v1/auth/login` | `POST /api/v1/auth/login` | Replace with direct call to `apiClient.post('/auth/login', { registrationNumber, password })`. | NOT MIGRATED |
| **66** | `logout()` | `App.tsx` (`HeaderBar`) | Sets `currentUser` to guest | `POST /api/v1/auth/logout` | Call `apiClient.post('/auth/logout')` and purge token. | NOT MIGRATED |
