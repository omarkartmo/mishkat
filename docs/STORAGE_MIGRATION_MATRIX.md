...
| **8** | `getCategories()` | `App.tsx` (initial state & sync) | `CategoryRepository` -> Central API | `GET /api/v1/categories` | Migrated to `CategoryRepository.getCategories()`. | **MIGRATED (Phase 1.7.3-A)** |
| **9** | `addCategory(cat)` | `App.tsx` (`CategoryManagerView`) | `CategoryRepository` -> Central API | `POST /api/v1/categories` | Migrated to `CategoryRepository.createCategory()`. | **MIGRATED (Phase 1.7.3-A)** |
| **10** | `updateCategory(id, updates)` | `App.tsx` (`CategoryManagerView`) | `CategoryRepository` -> Central API | `PUT /api/v1/categories/:id` | Migrated to `CategoryRepository.updateCategory()`. | **MIGRATED (Phase 1.7.3-A)** |
| **11** | `deleteCategoryWithReassign(catId, targetCatId)` | `App.tsx` (`CategoryManagerView`) | `CategoryRepository` -> Central API | `POST /api/v1/categories/:id/reassign-delete` or `DELETE /api/v1/categories/:id` | Migrated to `CategoryRepository.reassignAndDeleteCategory()`. | **MIGRATED (Phase 1.7.3-A)** |
