...
export const AdminRecoveryModal: React.FC<AdminRecoveryModalProps> = ({ onClose, onRecovered }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm" dir="rtl">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md shadow-2xl shadow-indigo-900/20 overflow-hidden flex flex-col animate-in fade-in zoom-in-95">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/50">
          ...
          <button
            onClick={onClose}
            className="text-slate-500 dark:text-slate-400 hover:text-white bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-700 p-2 rounded-xl transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">رقم قيد المشرف</label>
                <input
                  type="text"
                  value={registrationNumber}
                  onChange={(e) => setRegistrationNumber(e.target.value)}
                  placeholder="ADM-001"
                  className="w-full pl-4 pr-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 rounded-xl text-sm font-mono text-white placeholder-slate-600 transition-colors"
                  autoFocus
                />
              </div>
              <button
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">سؤال الأمان</label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">الإجابة</label>
                <input
                  type="text"
                  value={securityAnswer}
                  onChange={(e) => setSecurityAnswer(e.target.value)}
                  placeholder="أدخل الإجابة السرية..."
                  className="w-full pl-4 pr-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 rounded-xl text-sm text-white placeholder-slate-600 transition-colors"
                  autoFocus
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••••••"
                    className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 rounded-xl text-sm font-mono text-white placeholder-slate-600 transition-colors"
                  />
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="py-3 px-4 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 transition-all flex items-center justify-center cursor-pointer"
                >
