  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="p-5 sm:p-6 border-b border-slate-800 flex items-center justify-between bg-gradient-to-r from-slate-900 via-emerald-950/30 to-slate-900 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-mono border border-emerald-500/30 uppercase">
                  {book.format || 'PDF'}
                </span>
              </h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-300 font-medium mb-1.5">عنوان الكتاب *</label>
              <input
                type="text"
                required
                value={title}
                ...
                className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl px-3.5 py-2.5 text-slate-100 outline-none transition-all"
              />
            </div>

            <div>
              <label className="block text-slate-300 font-medium mb-1.5">المؤلف / المحقق</label>
              <input
                type="text"
                value={author}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-300 font-medium mb-1.5">التصنيف الموضوعي *</label>
              <select
                value={categoryId}
                onChange={(e) => setCategoryId(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl px-3.5 py-2.5 text-slate-100 outline-none transition-all cursor-pointer"
              >
                {categories.map((c) => (
            ...
            <div>
              <label className="block text-slate-300 font-medium mb-1.5 flex items-center justify-between">
                ...
                <span className="text-[10px] text-emerald-400 font-normal">مطابق لملف الـ PDF</span>
              </label>
              <div className="relative">
                <FileText className="w-4 h-4 text-slate-500 absolute right-3 top-1/2 -translate-y-1/2" />
                <input
                  type="number"
                  min={1}
                  value={pagesCount || ''}
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl pr-9 pl-3.5 py-2.5 text-slate-100 font-mono outline-none transition-all"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-300 font-medium mb-1.5">المصدر / جهة الإصدار</label>
              <div className="relative">
                <Globe2 className="w-4 h-4 text-slate-500 absolute right-3 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  value={sourceOrigin}
                  onChange={(e) => setSourceOrigin(e.target.value)}
                  placeholder="مثال: وزارة التربية والتعليم، مجمع الملك سلمان..."
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl pr-9 pl-3.5 py-2.5 text-slate-100 outline-none transition-all"
                />
              </div>
            </div>

            <div>
              <label className="block text-slate-300 font-medium mb-1.5">الكلمات الدلالية والوسوم</label>
              <div className="relative">
                <Tag className="w-4 h-4 text-slate-500 absolute right-3 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  value={tags}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl p-3.5 text-slate-100 leading-relaxed outline-none transition-all resize-none"
            />
          </div>

          {/* Delete Confirmation Banner (If requested) */}
          {showDeleteConfirm && (
            <div className="p-3.5 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-between gap-3 text-rose-300 animate-in fade-in duration-150">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="px-3.5 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-md cursor-pointer disabled:opacity-50"
                >
          ...
          <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-800 shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="flex items-center gap-1.5 px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-xl text-xs font-bold shadow-lg shadow-emerald-600/30 cursor-pointer transition-all"
              >
